...
<script>
    $(document).ready(function() {
        ...
        // Listen for changes in the video_link input
        $('input[name="video_link"]').on('input', function() {
            // If the video_link field is not empty, disable browse_video
            if ($(this).val().trim() !== '') {
                $('input[name="browse_video"]').prop('disabled', true);
            } else {
                // Otherwise, enable browse_video
                $('input[name="browse_video"]').prop('disabled', false);
            }
        });

        // Listen for changes in the browse_video input
        $('input[name="browse_video"]').on('change', function() {
            // If the browse_video field has a file, disable video_link
            if ($(this).prop('files').length > 0) {
                $('input[name="video_link"]').prop('disabled', true);
            } else {
                // Otherwise, enable video_link
                $('input[name="video_link"]').prop('disabled', false);
            }
        });
    });
</script>
